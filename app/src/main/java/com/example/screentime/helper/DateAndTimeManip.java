...
public class DateAndTimeManip {
    ...
    public static String getTimeStringFromIntegerTime(long time) {
        ...
        String Time = "";
        if (hours > 0) {
            Time = Integer.toString(hours) + " jam ";
        }
        if (minutes > 0) {
            Time += Integer.toString(minutes) + " menit ";
        }
        if (seconds > 0) {
            Time += Integer.toString(seconds) + " detik";
        }
    ...
    public static String getTimeInHoursAndMin(int time) {
        ...
        int min = time % 60;
        if (hour > 0) {
            timeString = Integer.toString(hour) + " Jam ";
        }
        if (min > 0) {
            timeString += Integer.toString(min) + " Menit ";
        }
        return timeString;
    }
